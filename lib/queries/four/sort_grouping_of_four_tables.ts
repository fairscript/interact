...
export class SortGroupingOfFourTables<T1, T2, T3, T4, K extends ValueRecord> {
    ...
    thenBy(sortBy: (key: K, first: AggregatableTable<T1>, second: AggregatableTable<T2>, third: AggregatableTable<T3>, count: () => Count) => K | Max | Min | Avg | Sum | Count): SortGroupingOfFourTables<T1, T2, T3, T4, K> {
        return new SortGroupingOfFourTables(addAscendingGroupOrder(this.statement, sortBy))
    }

    thenDescendinglyBy(sortBy: (key: K, first: AggregatableTable<T1>, second: AggregatableTable<T2>, third: AggregatableTable<T3>, count: () => Count) => K | Max | Min | Avg | Sum | Count): SortGroupingOfFourTables<T1, T2, T3, T4, K> {
