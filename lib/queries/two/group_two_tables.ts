...
import {RowSelectGenerator} from '../select_generators'
...
export class GroupTwoTables<T1, T2, K extends StringValueRecord> {
    ...
    aggregate<A extends StringValueRecord>(
        aggregation: (key: K, first: AggregatableTable<T1>, second: AggregatableTable<T2>) => EnforceNonEmptyRecord<A> & A): RowSelectGenerator<A> {
